...
ORDER BY
 ...
 select date::text as date, 
       sum( total_pending_solns) as  pending,
       sum(pending_solns_within_7days) as  pendingI,
       sum(pending_solns_7to15days) as  pendingII,
       sum(pending_solns_15to30days) as  pendingIII, 
       sum(pending_solns_monthlong) as  pendingIV,
       ARRAY_AGG(
        ROW( moderator, total_pending_solns,
    pending_solns_within_7days,
    pending_solns_7to15days,
    pending_solns_15to30days,
    pending_solns_monthlong,first_sol_date, last_sol_date
    )::text) AS data_array
    from pending_solution 
    group by date
